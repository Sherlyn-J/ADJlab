...
working_path = "D:/TCGA/" # 
cohorts = [ 'ACC','DLBC','LAML','UCS','BLCA','BRCA','CESC','COAD','ESCA','GBM','HNSC','KICH','KIRC','LGG','LIHC','LUAD','LUSC', 'OV', 'PAAD', 'PRAD','READ', 'SARC', 'SKCM', 'STAD', 'THCA', 'UCEC' ]

for cohort in cohorts:
    print( cohort, end="... " )
    clinical_fn = 'D:\TCGA\%s\%s_Clinical.csv'%( cohort, cohort )
    raw_fn = 'D:\TCGA\%s\%s_RawCounts.csv'%( cohort, cohort )
    ...
    df.to_csv( working_path + cohort + "/" + cohort + "_TPMCounts.csv" )


def corr_plot( gene_list, sample_list, title="" ):
    corr = df.loc[ sample_list, gene_list ].corr()
